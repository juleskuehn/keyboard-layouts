...
genKB = function(layout) {
  var i, j, k, key, keyboard, l, label, len, len1, len2, m, newRow, r, row;
  keyboard = '';
  for (r = i = 0, len = layout.length; i < len; r = ++i) {
    row = layout[r];
    newRow = '<div class="row" id="row' + r + '">';
    for (k = j = 0, len1 = row.length; j < len1; k = ++j) {
      key = row[k];
      newRow += '<div class="key" id="key' + r + '-' + k + '"><div class="keyinner">';
      if (typeof key === 'string') {
        newRow += '<div class="label">' + key.toUpperCase() + '</div>';
      } else {
        for (l = m = 0, len2 = key.length; m < len2; l = ++m) {
          label = key[l];
...
bind = function(source, target) {
  var fn, i, j, k, key, len, len1, r, row, shift;
  kp.reset();
  lowerShelf = [];
  for (r = i = 0, len = source.length; i < len; r = ++i) {
    row = source[r];
    fn = function(key, target, r, k) {
...
keyPress = function(key) {
  ...
  if (shiftPressed && (key.match(/^[a-z]+$/) || indexOf.call(lowerShelf, key) >= 0)) {
